...
file under `.crux/`. Project outputs and generated references are created later by install,
onboarding, and lazy-loading. No runtime required to read or understand what the system is doing.
...
- `workspace/` — generated at install / first boot
...
│
├── decisions/               generated project decisions — created after install or approval
...
│       └── to-{target}.jsonl
│
└── workspace/               generated after install, gitignored — all live state
...
Project-instance state and generated outputs should appear only after install or onboarding.

**Decisions live in decisions/, not docs/**
`.crux/docs/` should be generated after install or lazy-loading only when an agent needs them.
...
Project-specific `decisions/` files are generated after install or approval flow.
...
Lives in `workspace/MANIFEST.md`, not committed `.crux/`. It changes every session.
Generated after install or first boot and kept gitignored.
...
7. docs/{doc}.md                on demand, generate if missing
...
12. Update workspace/MANIFEST.md — add agent row, status: pending-onboard
```
